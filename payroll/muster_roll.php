...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($counter == 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $query2 = "SELECT count(total) FROM tbl_bene_deduct WHERE b_year ='$year' and b_month='$month'and emp_no='$emp_no' and type='deduction'";
      $result2 = mysqli_query($conn, $query2);
      if ($row2 = mysqli_fetch_assoc($result2)) {
        $total_deduct = $row2['count(total)'];
      }
      // get total benefits 
      $query2 = "SELECT count(total) FROM tbl_bene_deduct WHERE b_year ='$year' and b_month='$month'and emp_no='$emp_no' and type='benefit'";
      $result2 = mysqli_query($conn, $query2);
      if ($row2 = mysqli_fetch_assoc($result2)) {
        $total_benef = $row2['count(total)'];
      }
