<?php
	...
	if ($stmt = $con->prepare('SELECT count(*) as total FROM tbl_category WHERE category_name = ?')) {
		...
		if ($acc > 0) {
			...
			die ('That Category Item Exists');
			
		}
		
		else {
			// Username doesnt exists, insert new account
			if ($stmt = $con->prepare('INSERT INTO tbl_category (category_name) VALUES (?)')) {
				$stmt->bind_param('s', $_POST['modal_category_name']);
                $stmt->execute();
